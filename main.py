# main.py — Tensura World (upload command supports direct attach & reply-to-photo)
import os
import random
import sqlite3
import logging
...
async def upload_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not args_text:
        ...
        # in that case, reuse photo_handler logic to parse caption and save
        caption = update.message.caption or ""
        if caption:
            # delegate to photo_handler-like behavior
            data = {}
            for line in caption.splitlines():
                if ":" not in line:
                    continue
...
async def summon10(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not results:
        await update.message.reply_text("❌ No characters configured.")
        return
    # store results and add to inventory + exp
    tally = {}
    for ch in results:
        add_to_inventory(uid, ch[0]); add_exp(uid, 10)
        key = (ch[0], ch[1], ch[2])
