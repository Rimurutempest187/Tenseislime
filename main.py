...
async def upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not update.message.reply_to_message or not context.args:
        await update.message.reply_text("❌ Reply to photo with /upload ID|Name|Rarity|Faction|Power|Price")
        return
    ...
    conn.commit()
    await update.message.reply_text(f"✅ Character {name} saved")
...
async def do_summon(update, times):
    ...
    for _ in range(times):
        ...
        add_exp(uid, 10)
        await update.message.reply_photo(char[6], caption=format_char_row(char))
    conn.commit()
...
async def store(update, context):
    ...
    keyboard = [[InlineKeyboardButton("🛒 Buy", callback_data=f"buy_{char[0]}"),
                 InlineKeyboardButton("➡ Next", callback_data="next_store")]]
    await update.message.reply_photo(char[6], caption=format_char_row(char), reply_markup=InlineKeyboardMarkup(keyboard))
...
async def store_btn(update, context):
    ...
    if q.data.startswith("buy_"):
        ...
        ON CONFLICT(user_id,char_id) DO UPDATE SET count=count+1
        """, (uid, cid))
        conn.commit()
...
async def inventory(update, context):
    ...
    init_user(uid)
    c.execute("""
    SELECT characters.id,characters.name,characters.rarity,characters.faction,characters.power,characters.price,inventory.count
    FROM inventory
    ...
    for i,row in enumerate(items,1):
        msg += f"{i}. {format_char_row(row)} x{row[6]}\n\n"
...
async def addcoins(update, context):
    ...
    if not is_admin(uid):
        ...
        return
    if not update.message.reply_to_message or not context.args:
        await update.message.reply_text("❌ Reply to user with /addcoins <amount>")
        return
    ...
    conn.commit()
    await update.message.reply_text(f"✅ Added {amount} coins")
...
def main():
    ...
    # Photo filter (optional: for /upload)
